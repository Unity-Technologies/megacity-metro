...
namespace Unity.MegacityMetro.UI
{
    ...
    public class UISoundSettings : UISettingsTab
    {
        ...
        private void OnVolumeUpdated(ChangeEvent<int> value)
        {
            SetLogVolume(Mathf.Log((value.newValue + k_MinVolume) / 100f) * 20f + m_MaxVolume);
        }

        private void SetLogVolume(float volume)
        {
        ...
        private void OnSoundFXUpdated(ChangeEvent<int> value)
        {
            AudioMaster.Instance.soundFX.audioMixer.SetFloat("sound-fx",
                Mathf.Log((value.newValue + k_MinVolume) / 100f) * 20 + m_MaxSoundFX);
        }

        private void OnMusicUpdated(ChangeEvent<int> value)
        {
            AudioMaster.Instance.music.audioMixer.SetFloat("music",
                Mathf.Log((value.newValue + k_MinVolume) / 100f) * 20 + m_MaxMusic);
        }
